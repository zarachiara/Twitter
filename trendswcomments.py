...
def tweet_words(tweet):
    ...
    "*** YOUR CODE HERE ***"
    take_words = tweet['text']
    return extract_words(take_words)
...
def extract_words(text):
    ...
    "*** YOUR CODE HERE ***"
    for words in text:
        if words not in ascii_letters: # string ascii letters in the string module
            text = text.replace(words,' ')
    return text.split()  # Replace this line. Do I need to replace? 
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in list_words: 
        ...
        if has_sentiment(sentiment): # abstraction
            value = sentiment_value(sentiment)
            total = total + value
            count = count + 1
    if count == 0: 
        return make_sentiment(None)
    average = make_sentiment(total/count) # average all words in tweet
    return average
...
def find_state_center(polygons):
    ...
    for shape in polygons:
        centroid = find_centroid(shape) # abstraction! 
        area_i = centroid[2]
        c_ix = centroid[0]
        c_iy = centroid[1]
        x = x + (c_ix *area_i)
    ...
    # what is polygons? a list. I want to look through each element. 
...
def group_tweets_by_state(tweets):
    ...
    us_centers = {n: find_state_center(s) for n, s in us_states.items()}
    "*** YOUR CODE HERE ***"
    tweets_by_state = {n: [] for n in us_states.keys()}
    for tweet in tweets:
...
def most_talkative_states(term):
    ...
    tweet_count_by_state = [(n, len(s)) for n, s in tweets_by_state.items()]
    tweet_count_by_state.sort()
    tweet_count_by_state = sorted(tweet_count_by_state, key=lambda state: -state[1])
    return tweet_count_by_state[:5]
...
def average_sentiments(tweets_by_state):
    ...
    "*** YOUR CODE HERE ***"
    for name, tweets in tweets_by_state.items():
        count = 0
        total = 0
        for tweet in tweets:
            ...
            if has_sentiment(sentiment): # abstraction
                ...
                count = count + 1
        if count > 0: 
            average = total/count # average all sentiment in state
            averaged_state_sentiments[name] = average
...
def group_tweets_by_hour(tweets):
    ...
    for tweet in tweets:
        hour = tweet_time(tweet).time().hour
